 ...
 * 
 * Features:
 * - [OK] Model selection UI (Claude Sonnet 3.5, GPT-4o, Gemini 1.5 Pro)
 * - [OK] Cost tracking per API call
 * - [OK] Performance comparison dashboard
 * - [OK] Automatic fallback if provider fails
 * - [OK] Error handling and retry logic
 * 
...
// Backend version - uses process.env for API keys and in-memory storage for cost tracking

// Import context provider for building extraction context
const contextProvider = require('./context/contextProvider');

// Import LLM cache utility
const { 
  getCachedLLMResponse, 
  setCachedLLMResponse 
} = require('../utils/llmCache');
...
const getApiKey = (provider) => {
  const keyMap = {
    [API_PROVIDERS.ANTHROPIC]: process.env.ANTHROPIC_API_KEY,
    [API_PROVIDERS.OPENAI]: process.env.OPENAI_API_KEY,
    [API_PROVIDERS.GEMINI]: process.env.GOOGLE_API_KEY
  };
...
// In-memory storage for cost tracking (backend version)
let costTrackingStore = {
  totalCost: 0,
  byProvider: {},
  byTask: {},
  history: []
...
const initCostTracking = () => {
  // Already initialized with in-memory stores above
...
const recordCost = (provider, task, inputTokens, outputTokens, cost, duration, success = true, error = null) => {
  ...
  const tracking = costTrackingStore;
  const metrics = performanceMetricsStore;
  ...
  // Data is already in memory, no need to save to localStorage
 ...
 */
const getCostTracking = () => {
  initCostTracking();
  return costTrackingStore;
};
...
const resetCostTracking = () => {
  ...
  console.log('[OK] Cost tracking reset');
};
...
// In-memory model selection (backend version)
let selectedModelId = process.env.DEFAULT_LLM_MODEL || 'claude-sonnet-3.5';

/**
 * Get selected model configuration
 */
const getSelectedModel = () => {
  return PREMIUM_MODELS[selectedModelId] || PREMIUM_MODELS['claude-sonnet-3.5'];
};
...
const setSelectedModel = (modelId) => {
  ...
  console.log(`[OK] Selected model: ${PREMIUM_MODELS[modelId].name}`);
  return true;
};
 ...
 */
const getActiveLLMProvider = (task = null) => {
 ...
 */
const getModelConfig = (task = null, useFastModel = true) => {
 ...
 * 
 * Features:
 * - [OK] Uses selected premium model
 * - [OK] Automatic fallback if primary model fails
 * - [OK] Cost tracking for every API call
 ...
 */
const callLLMWithFallback = async (prompt, options = {}) => {
  const {
    task = 'general',
  ...
  if (enableCache) {
    ...
    if (cached) {
      console.log(`[LLM Cache] [OK] Cache hit for ${task}`);
      return cached;
    }
  }
  ...
  for (let i = 0; i < fallbackModels.length; i++) {
    ...
    const isPrimary = i === 0;
    console.log(`[LLM] ${isPrimary ? '[TARGET] Primary' : '[REFRESH] Fallback'}: ${model.name} for ${task}`);
    ...
    try {
      ...
      console.log(`[LLM] [OK] Success with ${model.name} | ${duration}ms | $${cost.toFixed(4)}`);
      ...
      console.error(`[LLM] [ERROR] Failed with ${model.name}:`, error.message);
      ...
      if (i < fallbackModels.length - 1 && enableFallback) {
        console.log(`[LLM] [REFRESH] Trying fallback model...`);
        continue;
      }
...
const checkBackendAvailable = async () => {
  ...
  try {
    ...
    console.log(`[Backend] ${backendAvailable ? '[OK] Available' : '[ERROR] Unavailable'} - Using ${backendAvailable ? 'secure proxy' : 'client-side keys'}`);
    return backendAvailable;
  } catch (error) {
    backendAvailable = false;
    console.log('[Backend] [ERROR] Unavailable - Using client-side keys');
    return false;
  }
};
...
const callAnthropicAPI = async (model, prompt, systemPrompt, options) => {
  ...
  // Backend: Use environment variable API key
  const apiKey = getApiKey(API_PROVIDERS.ANTHROPIC);
  if (!apiKey) {
...
const callOpenAIAPI = async (model, prompt, systemPrompt, options) => {
  ...
  // Backend: Use environment variable API key
  const apiKey = getApiKey(API_PROVIDERS.OPENAI);
  if (!apiKey) {
...
const callGeminiAPI = async (model, prompt, systemPrompt, options) => {
  ...
  // Backend: Use environment variable API key
  const apiKey = getApiKey(API_PROVIDERS.GEMINI);
  if (!apiKey) {
...
const callLLM = async (prompt, options = {}) => {
  ...
  try {
    ...
    if (enableCache && response) {
      const cacheKey = `${prompt}_${systemPrompt}_${task}_${useFastModel}`;
      setCachedLLMResponse(cacheKey, response);
 ...
 */
const extractWithLLM = async (notes, options = {}) => {
  const {
  ...
  if (context.pathology.primary !== 'general') {
    ...
    if (gradingScales && Object.keys(gradingScales).length > 0) {
      knowledgeContext += `\nGRADING SCALES:\n`;
  ...
  if (context.consultants?.present && context.consultants?.services?.length > 0) {
...
[SUCCESS] CHRONOLOGICAL DEDUPLICATION: Procedure mentioned 5 times = 1 procedure (find actual date)
[SUCCESS] NATURAL LANGUAGE INFERENCE: Use medical reasoning to deduce implicit information
[SUCCESS] MULTI-SOURCE SYNTHESIS: Integrate attending + resident + PT/OT + consultant notes
[SUCCESS] CONSULTANT NOTES ARE CRITICAL: When present, consultant notes (neurology, PT/OT, cardiology, etc.) provide specialty expertise and critical recommendations - prioritize their findings and integrate their assessments into the clinical picture
[SUCCESS] FUNCTIONAL STATUS: Extract mobility/independence from OT/PT descriptions (these are often ONLY in PT/OT consult notes)
[SUCCESS] CLINICAL EVOLUTION: Capture the narrative arc, not just discrete data points
[SUCCESS] CONSULTANT STYLES: Understand neurology (deficit-focused), PT (mobility-focused), cardiology (cardiac risk), ID (antibiotic stewardship), etc.
[SUCCESS] TEMPORAL INTELLIGENCE: "POD#3" → actual date, "improving" → better functional scores
...
Return ONLY the JSON object with no markdown formatting, no explanation, no code blocks:`;
  ...
  // [OK] NEW: Use enhanced LLM call with automatic fallback and cost tracking
 ...
 * Or Claude Haiku > GPT-4o-mini > Gemini Flash (fast)
 */
const generateSummaryWithLLM = async (extractedData, sourceNotes, options = {}) => {
  const {
    useFastModel = true, // PHASE 1: Use fast models by default
    preserveAllInfo = false, // PHASE 1: No truncation if true
    enableCache = true // PHASE 1: Enable caching
  ...
  console.log(`[ANALYSIS] Prompt optimization: Data ${JSON.stringify(extractedData).length} → ${JSON.stringify(conciseData).length} chars, Notes ${sourceNotes.length} → ${truncatedNotes.length} chars (preserveAllInfo: ${preserveAllInfo})`);
...
Generate comprehensive discharge summary:`;
  ...
  if (narrative) {
    ...
    console.error('[ERROR] LLM returned no narrative!');
  }
