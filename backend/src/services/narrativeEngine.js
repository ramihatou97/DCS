...
const { synthesizeMultiSourceNarrative } = require('../utils/narrativeSynthesis.js');
const { applyMedicalWritingStyle, validateMedicalWritingStyle } = require('../utils/medicalWritingStyle.js');
const { buildNarrativeWithTransitions, selectTransition } = require('../utils/narrativeTransitions.js');
...
} = require('../utils/narrativeTemplates.js');

// COMPLETENESS FIX: Import comprehensive section generators
const { ensureAllSectionsGenerated } = require('../utils/narrativeSectionGenerators.js');
...
} = require('../utils/specificNarrativeGenerators.js');
 ...
 * });
 */
const generateNarrative = async (extractedData, sourceNotes = '', options = {}) => {
  const {
  ...
  // [OK] CRITICAL FIX: Normalize extracted data to prevent "map is not a function" errors
  ...
  if (applyLearnedPatterns) {
    try {
      ...
      if (Object.keys(learnedPatterns).length > 0) {
        console.log(`[DATA] Applying ${Object.keys(learnedPatterns).length} learned narrative patterns`);
      }
    } catch (error) {
 ...
 */
const generateTemplatedNarrative = (extractedData, pathologyType) => {
  // [OK] Data will be normalized inside generateNarrative
 ...
 */
const generateConciseSummary = (extractedData) => {
  // [OK] CRITICAL FIX: Normalize data first
  const extracted = normalizeExtractedData(extractedData);
 ...
 */
const formatNarrativeForExport = (narrative, options = {}) => {
  const { 
    includeHeaders = true,
    sectionSeparator = '\n\n',
...
const validateAndCompleteSections = (narrative, extracted, intelligence = null) => {
  ...
  const sectionRules = [
    ...
    },
    {
      ...
      template: () => {
        ...
        // [OK] SAFETY: complications is now guaranteed to be an array
  ...
  if (sectionsFixed > 0) {
    console.log(`[Narrative Validation] [OK] Fixed/completed ${sectionsFixed} narrative sections`);
  } else {
    console.log('[Narrative Validation] [OK] All sections adequate, no fixes needed');
  }
...
const parseLLMNarrative = (llmText) => {
  ...
  console.log('[SEARCH] LLM Response (first 500 chars):', llmText.substring(0, 500));
  console.log('[SEARCH] LLM Response length:', llmText.length);
  ...
  sectionHeaders.forEach(({ key, patterns }) => {
    ...
    for (const pattern of patterns) {
      ...
      for (const regex of regexPatterns) {
        ...
        if (match && match[1] && match[1].trim()) {
          ...
          console.log(`[OK] Extracted ${key}: ${sectionContent.substring(0, 100)}...`);
    ...
    if (!sectionContent) {
      console.log(`[WARN]Could not extract ${key} from LLM response`);
    }
  ...
  for (const pattern of medicationPatterns) {
    ...
    for (const regex of regexPatterns) {
      ...
      if (match && match[1] && match[1].trim()) {
        ...
        console.log(`[OK] Extracted medications: ${medicationContent.substring(0, 100)}...`);
        break;
      }
    }
  ...
  if (!medicationContent) {
    console.log(`[WARN]Could not extract medications from LLM response`);
  }
  ...
  if (extractedSections === 0) {
    ...
    if (fallbackExtracted === 0) {
      console.error('[ERROR] Even fallback parsing failed. LLM response may be in unexpected format.');
      ...
      if (llmText.length > 100 && /patient|admission|discharge|procedure|diagnosis/i.test(llmText)) {
        console.log('[REFRESH] Using entire LLM response as hospital course');
