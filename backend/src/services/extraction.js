...
const { PATHOLOGY_PATTERNS, detectPathology } = require('../config/pathologyPatterns.js');
const { EXTRACTION_TARGETS, CONFIDENCE } = require('../config/constants.js');
const { parseFlexibleDate, normalizeDate } = require('../utils/dateHelpers.js');
const {
  cleanText,
  preprocessClinicalNote
} = require('../utils/textProcessing.js');
const { extractAnticoagulation } = require('../utils/anticoagulationTracker.js');
const { extractDischargeDestination } = require('../utils/dischargeDestinations.js');
...
const { validateComplicationExtraction } = require('../utils/negationDetection.js');

// Phase 1 Enhancement: Temporal Qualifiers
const { extractTemporalQualifier } = require('../utils/temporalQualifiers.js');

// Phase 1 Enhancement: Source Quality Assessment
const { assessSourceQuality, calibrateConfidence } = require('../utils/sourceQuality.js');
...
const {
  ...
  resolveRelativeDate
} = require('../utils/temporalExtraction.js');
const {
  deduplicateBySemanticSimilarity,
  getDeduplicationStats
} = require('../utils/semanticDeduplication.js');
const { calculateCombinedSimilarity } = require('../utils/ml/similarityEngine.js');

// Phase 1 Step 6: Pathology Subtypes Detection
const { detectPathologySubtype } = require('../utils/pathologySubtypes.js');

// Phase 2: Clinical Intelligence & Context Enhancement
const { buildCausalTimeline } = require('../utils/causalTimeline.js');
const { trackTreatmentResponses } = require('../utils/treatmentResponse.js');
const { analyzeFunctionalEvolution } = require('../utils/functionalEvolution.js');
const { extractClinicalRelationships } = require('../utils/relationshipExtraction.js');
const { calculateQualityMetrics } = require('./qualityMetrics.js');

// Phase 0: Feature flags for gradual rollout
const { isFeatureEnabled, FEATURE_FLAGS } = require('../utils/featureFlags.js');
...
} = require('./comprehensiveExtraction.js');
 ...
 */
const deduplicateNotesAsync = async (notes, options = {}) => {
  // TODO: Replace with worker_threads for Node.js when needed
  // For now, use synchronous deduplication from deduplication service
  const startTime = Date.now();
  
  try {
    const deduplication = require('../services/extraction/deduplication.js');
    const result = await deduplication.deduplicateNotes(notes, options);
    const duration = Date.now() - startTime;
    
    console.log(`[SUCCESS] Deduplication completed in ${duration}ms (synchronous fallback)`);
    return result;
  } catch (error) {
 ...
 * });
 */
const extractMedicalEntities = async (notes, options = {}) => {
  const {
    learnedPatterns = [],
...
const extractWithPatterns = async (combinedText, noteArray, pathologyTypes, options) => {
  ...
  try {
    ...
    console.log(`[ANALYSIS] Source Quality: ${sourceQuality.grade} (${(sourceQuality.overallScore * 100).toFixed(1)}%)`);

    // Log quality issues if any
    if (sourceQuality.issues.length > 0) {
      console.log(`[WARNING]️ Quality Issues: ${sourceQuality.issues.map(i => i.factor).join(', ')}`);
    }
  } catch (error) {
  ...
  if (sourceQuality && includeConfidence) {
    try {
      ...
      console.log(`[OK] Confidence scores calibrated based on source quality`);
    } catch (error) {
  ...
  if (pathologyTypes.length > 0) {
    ...
    for (const pathType of pathologyTypes) {
      try {
        ...
        // Defensive check: ensure subtype and subtype.details exist
      ...
      try {
        ...
        if (validation.valid) {
          ...
          if (validation.reason) {
            console.log(`[WARNING]️ Complication "${complicationName}" filtered: ${validation.reason}`);
          }
        }
      } catch (error) {
