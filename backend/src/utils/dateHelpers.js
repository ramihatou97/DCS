 ...
 * 
 * Backend version - CommonJS module
 */
 ...
 */
const formatMedicalDate = (date, formatString = 'MM/dd/yyyy') => {
  if (!date) return '';
 ...
 */
const calculatePOD = (surgeryDate, currentDate) => {
 ...
 */
const parseRelativeTime = (text, referenceDate) => {
  const patterns = [
    /(\d+)\s+days?\s+ago/i,
    /(\d+)\s+weeks?\s+ago/i,
 ...
 */
const getRelativeTime = (date, referenceDate = new Date()) => {
