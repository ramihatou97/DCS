...
const NEW_EVENT_PATTERNS = {
  ...
  // Section headers indicating new events (high confidence)
 ...
 */
const extractPODContext = (text, position = 0) => {
  if (!text) {
 ...
 */
const classifyEventType = (context, temporalQualifier = {}) => {
  const refDetection = detectReferencePhrase(context);
 ...
 */
const resolveRelativeDate = (relativeRef, referenceDates) => {
 ...
 */
const linkReferencesToEvents = (references, events, similarityFn) => {
 ...
 */
const isReferenceToPatient = (text) => {
